<x-app-layout>
    <div class="py-12 px-4 sm:px-6 lg:px-8 max-w-4xl mx-auto">
        ...
        <div class="mb-10 flex items-center justify-between">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-slate-500 dark:text-slate-400 text-sm font-medium mt-1">Actualiza la información de la
                        actividad</p>
                </div>
            </div>
        ...
        <div
            ...
            <form action="{{ route('event.update', $event->eventID) }}" method="POST" enctype="multipart/form-data"
                    ...
                    <div class="space-y-6">
                        <div class="space-y-2">
                            ...
                            <input type="date" name="fdate" id="fdate"
                                value="{{ old('fdate', $event->fdate) }}" required
                                class="w-full px-4 py-3.5 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 transition-all text-slate-700 dark:text-slate-200 outline-none font-bold">
                        </div>
                        <div class="space-y-2">
                            <label for="ftime"
                            ...
                            <input type="time" name="ftime" id="ftime"
                                value="{{ old('ftime', $event->ftime) }}" required
                                class="w-full px-4 py-3.5 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 transition-all text-slate-700 dark:text-slate-200 outline-none font-bold">
                        </div>
                    </div>
                    ...
                    <div class="space-y-6">
                        <div class="space-y-2">
                            ...
                            <input type="date" name="tdate" id="tdate"
                                value="{{ old('tdate', $event->tdate) }}" required
                                class="w-full px-4 py-3.5 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 transition-all text-slate-700 dark:text-slate-200 outline-none font-bold">
                        </div>
                        <div class="space-y-2">
                            <label for="ttime"
                            ...
                            <input type="time" name="ttime" id="ttime"
                                value="{{ old('ttime', $event->ttime) }}" required
                                class="w-full px-4 py-3.5 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 transition-all text-slate-700 dark:text-slate-200 outline-none font-bold">
                        </div>
                    </div>
                </div>
                ...
                <div class="space-y-2">
                    <label
                        class="block text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest pl-1">Cambiar
                        Imagen del Evento</label>
                    <div class="w-full relative group">
                        @if ($event->photo)
                            <div
                                class="mb-4 w-40 h-40 rounded-2xl overflow-hidden shadow-lg border-4 border-white dark:border-slate-700">
                                <img src="{{ asset('uploads/images/' . $event->photo) }}"
                                    class="w-full h-full object-cover">
                            </div>
                        @endif
                        <label for="photo"
                            class="flex flex-col items-center justify-center w-full h-32 rounded-2xl bg-slate-50 dark:bg-slate-900 border-2 border-dashed border-slate-200 dark:border-slate-700 hover:border-blue-500 dark:hover:border-blue-400 transition-all cursor-pointer">
                            <div class="flex flex-col items-center justify-center pt-5 pb-6">
                                <i
                                    class="ti ti-upload text-3xl text-slate-300 dark:text-slate-600 group-hover:text-blue-500"></i>
                                <p class="text-[10px] text-slate-400 mt-2">Selecciona una nueva imagen para reemplazar
                                    la actual</p>
                            </div>
                ...
                <div class="space-y-2">
                    ...
                    <textarea name="details" id="details" rows="6" required
                        class="w-full px-4 py-3.5 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 transition-all text-slate-700 dark:text-slate-200 outline-none font-bold resize-none">{{ old('details', $event->details) }}</textarea>
                </div>
