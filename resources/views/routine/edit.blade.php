    ...
    <div class="py-10 px-4 sm:px-6 lg:px-8 w-full max-w-5xl mx-auto">
        ...
        <div class="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div>
                ...
                <p
                    class="text-slate-500 dark:text-slate-400 text-sm font-medium mt-1 uppercase tracking-tighter text-balance">
                    {{ __('Actualiza la información de la sesión para :subject de :class', ['subject' => $routine->subject->subject, 'class' => $routine->class->classes]) }}
                </p>
            </div>
        </div>
        ...
        <div
            ...
            <form action="{{ route('routine.update', $routine->routineID) }}" method="POST" class="p-8 md:p-12">
                @csrf
                @method('PUT')
                                    ...
                                    <select name="classesID" id="classesID"
                                        ...
                                        @foreach ($classes as $class)
                                            <option value="{{ $class->classesID }}"
                                                {{ old('classesID', $routine->classesID) == $class->classesID ? 'selected' : '' }}>
                                                {{ $class->classes }}
                                            </option>
                                        @endforeach
                                    </select>
                                    ...
                                    <select name="sectionID" id="sectionID"
                                        ...
                                        @foreach ($sections as $section)
                                            <option value="{{ $section->sectionID }}"
                                                {{ old('sectionID', $routine->sectionID) == $section->sectionID ? 'selected' : '' }}>
                                                {{ $section->section }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="relative group">
                                    ...
                                    <select name="day" id="day"
                                        ...
                                        @foreach ($days as $key => $label)
                                            <option value="{{ $key }}"
                                                {{ old('day', $routine->day) == $key ? 'selected' : '' }}>
                                                {{ $label }}
                                            </option>
                                        @endforeach
                                    </select>
