<x-app-layout>
    <div class="py-8 px-4 sm:px-6 lg:px-8 w-full mx-auto">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8">
            <div>
                <h1 class="text-3xl font-bold text-slate-900 dark:text-white tracking-tight">Gestión de Exámenes</h1>
                <p class="text-slate-500 dark:text-slate-400 mt-1">Administre las evaluaciones y fechas importantes por
                    periodo.</p>
            </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            @forelse($exams as $exam)
                <div
                    class="group relative p-6 rounded-3xl bg-white dark:bg-slate-800/30 border border-slate-200 dark:border-slate-700/50 shadow-sm dark:shadow-none backdrop-blur-xl hover:shadow-md dark:hover:bg-slate-800/50 hover:border-violet-500/30 transition-all duration-300">
                    <!-- Date Badge -->
                    <div class="absolute top-6 right-6 flex flex-col items-center">
                        <span
                            class="text-2xl font-black text-slate-800 dark:text-slate-200">{{ \Carbon\Carbon::parse($exam->date)->format('d') }}</span>
                        <span
                    ...
                    <div class="mb-6 pr-12">
                        ...
                        <h3
                            class="text-lg font-bold text-slate-900 dark:text-white mb-1 group-hover:text-violet-600 dark:group-hover:text-violet-400 transition-colors">
                            {{ $exam->exam }}</h3>
                    ...
                    <div
                        ...
                        <div class="flex items-center gap-2">
                            <a href="{{ route('exam.edit', $exam->examID) }}"
                                class="w-8 h-8 rounded-lg bg-slate-50 dark:bg-slate-900/50 hover:bg-violet-500/20 text-slate-500 dark:text-slate-400 hover:text-violet-600 dark:hover:text-violet-400 flex items-center justify-center transition-all"
                                title="Editar">
                            ...
                            <form action="{{ route('exam.destroy', $exam->examID) }}" method="POST"
                                ...
                                @method('DELETE')
                                <button type="submit"
                                    class="w-8 h-8 rounded-lg bg-slate-50 dark:bg-slate-900/50 hover:bg-rose-500/20 text-slate-500 dark:text-slate-400 hover:text-rose-600 dark:hover:text-rose-400 flex items-center justify-center transition-all"
                                    title="Eliminar">
            ...
            @empty
                <div class="col-span-full py-12 text-center">
                    ...
                    <h3 class="text-lg font-bold text-slate-800 dark:text-white mb-2">No hay exámenes programados</h3>
