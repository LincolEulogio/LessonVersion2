    ...
    <div class="py-10 px-4 sm:px-6 lg:px-8 w-full max-w-[1600px] mx-auto">
        ...
        <div class="mb-12 flex flex-col md:flex-row md:items-center justify-between gap-8">
            <div class="space-y-2">
                ...
                <p class="text-slate-500 dark:text-slate-400 text-xs font-bold uppercase tracking-[0.3em] ml-5">
                    {{ __('Planificación y Contenido por Materia') }}
                </p>
            </div>

            @if ($user && $user->hasPermission('tema_add'))
        ...
        <div class="mb-10 flex flex-col lg:flex-row gap-6 items-stretch">
            ...
            <div
                ...
                <form action="{{ route('topic.index') }}" method="GET" id="filter-form"
                    ...
                    <div class="relative group/search">
                        ...
                        <input type="text" name="search" id="search" value="{{ $search ?? '' }}"
                            placeholder="{{ __('Busca por título o descripción...') }}"
            ...
            <div
                ...
                <div
                    ...
                    <i class="ti ti-books text-[140px]"></i>
                </div>
                <div class="relative z-10 flex flex-col justify-between h-full">
                    <p class="text-[10px] font-black uppercase tracking-[0.2em] opacity-80">{{ __('Total Temas') }}</p>
                    <div class="flex items-end gap-3 mt-4">
                        <span class="text-5xl font-black leading-none tracking-tighter">{{ $topics->total() }}</span>
        ...
        <div
                ...
                <table class="w-full text-left border-separate border-spacing-0">
                    <thead>
                        <tr class="bg-slate-50/50 dark:bg-slate-900/50">
                            ...
                            <th
                                class="px-8 py-6 text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.3em] border-b border-slate-100 dark:border-slate-700/50 underline decoration-emerald-500/30 underline-offset-4">
                                {{ __('Materia') }}
                            </th>
                            <th
                            ...
                            <tr
                                ...
                                <td class="px-8 py-7">
                                    <div class="flex flex-col gap-1 max-w-sm">
                                        ...
                                        <p
                                            class="text-[11px] font-medium text-slate-400 dark:text-slate-500 line-clamp-2">
                                            {{ $topic->description }}
                                        </p>
                                    </div>
                                </td>
                                <td class="px-8 py-7">
                                    <div class="flex items-center gap-2">
                                        <div
                                            class="w-8 h-8 rounded-lg bg-emerald-500/10 flex items-center justify-center text-emerald-600 dark:text-emerald-400 font-black text-[10px]">
                                            {{ substr($topic->classes->classes ?? 'C', 0, 1) }}
                                        </div>
                                        <span class="text-sm font-bold text-slate-700 dark:text-slate-200">
                                            {{ $topic->classes->classes ?? __('Sin Clase') }}
                                        </span>
                                    </div>
                                </td>
                                <td class="px-8 py-7">
                                    <span
                                        class="px-4 py-2 rounded-xl bg-slate-100 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 text-xs font-black uppercase tracking-tighter">
                                        {{ $topic->subject->subject ?? __('General') }}
                                    </span>
                                </td>
                                <td class="px-8 py-7">
                                    <div
                                        ...
                                        @if ($user && $user->hasPermission('tema_view'))
                                            <a href="{{ route('topic.show', $topic->topicID) }}"
                                        ...
                                        @endif
                                        @if ($user && $user->hasPermission('tema_delete'))
                                            <button type="button"
                                                onclick="confirmDelete('{{ $topic->topicID }}', '{{ $topic->title }}')"
                                            ...
                                            <form id="delete-form-{{ $topic->topicID }}"
                                                action="{{ route('topic.destroy', $topic->topicID) }}" method="POST"
                                                class="hidden">
                                                @csrf
                                                @method('DELETE')
                                            </form>
                                ...
                                <td colspan="5" class="px-8 py-24 text-center">
                                    <div class="flex flex-col items-center gap-6">
                                        <div
                                            ...
                                            <i class="ti ti-books-off text-6xl"></i>
                                        </div>
                                        <div class="space-y-1">
                                            <p
                                                class="text-xl font-black text-slate-800 dark:text-slate-200 tracking-tight">
                                                {{ __('No se encontraron temas') }}
                                            </p>
                                            <p class="text-slate-400 dark:text-slate-500 text-sm font-medium">
                                                {{ __('Intenta con otros filtros o registra un nuevo tema para el plan de estudios.') }}
                                            </p>
                                        </div>
                                    </div>
                                </td>
            ...
            <div
                ...
                <div class="flex flex-col md:flex-row items-center gap-3">
                    ...
                    <div class="flex items-center gap-2">
                        <span
                            class="px-3 py-1 bg-white dark:bg-slate-800 rounded-lg text-xs font-black text-emerald-600 border border-slate-200 dark:border-slate-700 shadow-sm">{{ $topics->firstItem() ?? 0 }}</span>
                        <span class="text-slate-300 dark:text-slate-700">—</span>
                        <span
                            class="px-3 py-1 bg-white dark:bg-slate-800 rounded-lg text-xs font-black text-emerald-600 border border-slate-200 dark:border-slate-700 shadow-sm">{{ $topics->lastItem() ?? 0 }}</span>
                    </div>
                    <span
                        class="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">{{ __('de') }}</span>
                    <span class="text-sm font-black text-slate-900 dark:text-white">{{ $topics->total() }}</span>
                    <span
