<x-app-layout>
    <div class="py-12 px-4 sm:px-6 lg:px-8 max-w-[95%] mx-auto">
        <!-- Header & Filters -->
        <h1 class="text-3xl font-black text-white tracking-tight mb-8 flex items-center gap-3">
            <span
                class="w-10 h-10 rounded-xl bg-orange-500/10 flex items-center justify-center text-orange-400 border border-orange-500/20">
                <i class="ti ti-checklist text-xl"></i>
            </span>
            Registro de Notas
        </h1>

        <div class="mb-8 p-6 rounded-3xl bg-slate-800/30 border border-slate-700/50 backdrop-blur-xl shadow-2xl">
            <form action="{{ route('mark.add') }}" method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-6 items-end">
                <div class="space-y-2">
                    <label class="text-[10px] font-bold text-slate-500 uppercase tracking-widest ml-1">Clase</label>
                    <select name="classesID" onchange="this.form.submit()"
                        class="w-full bg-slate-900/50 border border-slate-700/50 rounded-xl px-4 py-2.5 text-slate-200 focus:border-orange-500/50 focus:ring-2 focus:ring-orange-500/10 transition-all outline-none">
                ...
                <div class="space-y-2">
                    <label class="text-[10px] font-bold text-slate-500 uppercase tracking-widest ml-1">Sección</label>
                ...
                <div class="space-y-2">
                    <label class="text-[10px] font-bold text-slate-500 uppercase tracking-widest ml-1">Materia</label>
                ...
                <div class="space-y-2">
                    <label class="text-[10px] font-bold text-slate-500 uppercase tracking-widest ml-1">Examen</label>
        ...
        @if (isset($students) && count($students) > 0)
            <div
                class="rounded-3xl bg-slate-800/30 border border-slate-700/50 backdrop-blur-xl overflow-hidden shadow-2xl">
                <div class="overflow-x-auto">
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-slate-900/50 text-[10px] font-black text-slate-500 uppercase tracking-widest">
                                <th class="p-6 min-w-[50px] text-center border-b border-r border-slate-700/50">#</th>
                                <th class="p-6 min-w-[250px] border-b border-r border-slate-700/50">Estudiante</th>
                                @foreach ($mark_percentages as $percentage)
                                    <th class="p-4 text-center border-b border-r border-slate-700/50 min-w-[120px]">
                                        {{ $percentage->markpercentage }}
                                        <span
                                            class="block text-[9px] text-orange-400 mt-1">({{ $percentage->markpercentage_numeric }}%)</span>
                                    </th>
                                @endforeach
                            </tr>
                        </thead>
                        <tbody class="divide-y divide-slate-700/30">
                            @foreach ($students as $index => $student)
                                <tr class="group hover:bg-slate-800/50 transition-colors">
                                    <td class="p-6 text-center text-slate-500 font-bold border-r border-slate-700/30">
                                        {{ $index + 1 }}
                                    </td>
                                    <td class="p-6 border-r border-slate-700/30">
                                        <div class="flex items-center gap-4">
                                            <div
                                                class="w-10 h-10 rounded-full bg-slate-700 overflow-hidden ring-2 ring-slate-600">
                                            ...
                                            <div>
                                                <div
                                                    ...
                                                    {{ $student->name }}</div>
                                                <div class="text-[10px] text-slate-500 uppercase tracking-wider">ID:
                                                    {{ $student->studentID }}</div>
                                            </div>
                                        </div>
                                    </td>
                                    @foreach ($mark_percentages as $percentage)
                                        <td class="p-3 border-r border-slate-700/30 text-center">
                                            <input type="number" name="mark_value"
                                                ...
                                                class="mark-input w-20 text-center bg-slate-900/50 border border-slate-700 rounded-lg py-2 text-slate-200 font-bold focus:border-orange-500 focus:ring-2 focus:ring-orange-500/20 outline-none transition-all placeholder-slate-600"
                                                placeholder="-">
                                        </td>
                                    @endforeach
                                </tr>
        ...
        @elseif(request('classesID'))
            <div class="mt-8 p-12 text-center rounded-3xl border-4 border-dashed border-slate-800/50 bg-slate-900/20">
                <div
                    class="w-24 h-24 rounded-full bg-slate-800 flex items-center justify-center text-slate-600 mx-auto mb-6">
                    <i class="ti ti-users-off text-4xl"></i>
                </div>
                <h3 class="text-xl font-bold text-white mb-2">No se encontraron estudiantes</h3>
                <p class="text-slate-500 max-w-sm mx-auto">Asegúrese de haber seleccionado todos los filtros
