        ...
        <div class="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div>
                ...
                <p class="text-slate-500 dark:text-slate-400 text-sm font-medium mt-1 uppercase tracking-tighter">
                    {{ __('Modifica los plazos, la materia o las instrucciones de la tarea') }}
                </p>
            </div>
        </div>
                        ...
                        <div class="space-y-3">
                            ...
                            <div class="relative group">
                                ...
                                <input type="date" name="deadlinedate" id="deadlinedate"
                                    value="{{ old('deadlinedate', $assignment->deadlinedate) }}"
                                    class="w-full pl-14 pr-6 py-4 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700/50 rounded-2xl text-slate-700 dark:text-slate-200 focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500 transition-all outline-none font-medium text-sm leading-relaxed">
                            </div>
                                ...
                                <select name="classesID" id="classesID"
                                    ...
                                    @foreach ($classes as $class)
                                        <option value="{{ $class->classesID }}"
                                            {{ old('classesID', $assignment->classesID) == $class->classesID ? 'selected' : '' }}>
                                            {{ $class->classes }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="space-y-3">
                            ...
                            <div class="relative group">
                                ...
                                <select name="subjectID" id="subjectID"
                                    class="w-full pl-14 pr-12 py-4 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700/50 rounded-2xl text-slate-700 dark:text-slate-200 focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500 transition-all outline-none appearance-none cursor-pointer font-medium text-sm leading-relaxed">
                                    <option value="" disabled>{{ __('Selecciona una materia') }}</option>
                                    @foreach ($subjects as $subject)
                                        <option value="{{ $subject->subjectID }}"
                                            {{ old('subjectID', $assignment->subjectID) == $subject->subjectID ? 'selected' : '' }}>
                                            {{ $subject->subject }}
                                        </option>
                                    @endforeach
                                </select>
                                ...
                                <div
                                    ...
                                    <div class="flex items-center gap-4">
                                        ...
                                        <div class="flex flex-col">
                                            ...
                                            <span
                                                class="text-sm font-bold text-slate-700 dark:text-slate-200">{{ $assignment->originalfile }}</span>
                                        </div>
                                    </div>
                        ...
                        <div class="space-y-3 md:col-span-2">
                            ...
                            <div class="relative group">
                                ...
                                <textarea name="description" id="description" rows="5"
                                    ...
                                    class="w-full pl-14 pr-6 py-4 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700/50 rounded-2xl text-slate-700 dark:text-slate-200 focus:ring-4 focus:ring-emerald-500/10 focus:border-emerald-500 transition-all outline-none resize-none font-medium text-sm leading-relaxed">{{ old('description', $assignment->description) }}</textarea>
                            </div>
