<x-app-layout>
    <div class="py-12 px-4 sm:px-6 lg:px-8 max-w-2xl mx-auto">
        ...
        <div class="mb-10 flex items-center gap-4">
            ...
            <div>
                <h1 class="text-3xl font-black text-slate-900 dark:text-white tracking-tight">Nuevo Rol de Usuario</h1>
                <p class="text-slate-500 dark:text-slate-400 text-sm font-medium mt-1">Crea una nueva categoría de acceso
                </p>
            </div>
        </div>
        ...
        <div
            ...
            <form action="{{ route('usertype.store') }}" method="POST" class="space-y-8 relative">
                @csrf

                <div class="space-y-4">
                    <x-form.label for="usertype" class="pl-1">Nombre del Rol</x-form.label>
                    <x-form.input name="usertype" id="usertype" placeholder="Ej: Bibliotecario, Coordinador..."
                        required />
                    <p class="text-[10px] text-slate-400 font-medium pl-1 leading-relaxed">Nota: El nombre del rol se
                        utilizará para definir permisos específicos más adelante.</p>
                </div>
