    ...
    <div class="py-8 px-4 sm:px-6 lg:px-8 max-w-5xl mx-auto">
        ...
        <div
            ...
            <div class="flex items-center gap-6 text-center md:text-left">
                <div
                    class="w-20 h-20 rounded-2xl bg-cyan-600/10 dark:bg-cyan-600/20 flex items-center justify-center text-cyan-600 dark:text-cyan-400 text-3xl shadow-inner">
                    <i class="ti ti-layers-linked"></i>
                </div>
                <div>
                    <h1 class="text-3xl font-bold text-slate-900 dark:text-white tracking-tight">{{ $section->section }}
                    </h1>
                    <div class="mt-2 flex items-center gap-3 justify-center md:justify-start">
                        <span
                            class="px-3 py-1 bg-cyan-600/10 dark:bg-cyan-600/20 text-cyan-600 dark:text-cyan-400 border border-cyan-500/10 dark:border-cyan-500/20 rounded-lg text-[10px] font-bold uppercase tracking-wider">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8 text-sm">
            ...
            <div class="md:col-span-2 space-y-8">
                ...
                <div
                    ...
                    <h2
                        ...
                        <i class="ti ti-info-circle text-cyan-600 dark:text-cyan-400"></i>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-y-8 gap-x-12 relative z-10">
                        <div>
                            <span
                                class="block text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest">Docente
                                Mentor</span>
                            <div class="mt-2 flex items-center gap-3">
                                ...
                                <span
                                    class="text-slate-700 dark:text-slate-200 font-bold">{{ $section->teacher_name ?? 'No asignado' }}</span>
                            </div>
                        </div>
                        <div>
                            <span
                                class="block text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest">Capacidad
                                Autorizada</span>
                            <div class="mt-2 flex items-center gap-3">
                                <span
                                    class="text-2xl font-bold text-cyan-600 dark:text-cyan-400 tracking-tighter">{{ $section->capacity }}</span>
                                <span class="text-slate-500 font-medium">Estudiantes máx.</span>
                            </div>
                        </div>
                        <div class="sm:col-span-2 text-sm italic">
                            <span
                                class="block text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mt-2 px-1">Notas
                                y Observaciones</span>
                            <p
                                class="text-slate-700 dark:text-slate-200 mt-2 block font-medium leading-relaxed bg-slate-50 dark:bg-slate-900/30 p-5 rounded-2xl border border-slate-100 dark:border-slate-700/30">
                ...
                <div
                    class="p-8 rounded-3xl bg-white dark:bg-cyan-600/5 border border-slate-200 dark:border-cyan-500/10 shadow-sm dark:shadow-none backdrop-blur-sm opacity-60">
                    <h2
                        class="text-xl font-bold text-slate-900 dark:text-white mb-6 border-b border-slate-100 dark:border-slate-700/50 pb-4 flex items-center gap-3">
                        <i class="ti ti-users-group text-cyan-600 dark:text-cyan-400"></i>
            ...
            <div class="space-y-6">
                ...
                <div
                    ...
                    <h3
                        class="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em] mb-6 border-b border-slate-100 dark:border-slate-700/30 pb-3">
                        Metadata</h3>
                    <div class="space-y-4">
                        <div class="flex items-center justify-between text-xs">
                            <span
                                class="text-slate-500 dark:text-slate-400 font-bold uppercase tracking-widest text-[10px]">Creado
                                por</span>
                            <span
                                class="text-slate-700 dark:text-slate-200 font-medium">{{ $section->create_username ?? 'Sistema' }}</span>
                        </div>
                        <div class="flex items-center justify-between text-xs">
                            <span
                                class="text-slate-500 dark:text-slate-400 font-bold uppercase tracking-widest text-[10px]">Modificado</span>
                            <span
                                class="text-slate-500 dark:text-slate-300 font-mono">{{ \Carbon\Carbon::parse($section->modify_date)->format('d/m/Y H:i') }}</span>
                        </div>
                    </div>
                </div>
                ...
                <div
                    class="p-6 rounded-3xl bg-slate-50 dark:bg-gradient-to-br dark:from-cyan-600/10 dark:to-transparent border border-slate-200 dark:border-cyan-500/20 backdrop-blur-md relative overflow-hidden">
                    <i class="ti ti-school absolute -bottom-4 -right-4 text-7xl text-cyan-500/10 rotate-12"></i>
                    <h4 class="text-slate-900 dark:text-white font-bold mb-2">Clase Superior</h4>
                    <p class="text-slate-400 text-xs">Esta sección pertenece al nivel académico:</p>
                    <div
                        class="mt-4 p-3 bg-white dark:bg-slate-900/50 rounded-xl border border-slate-100 dark:border-slate-700/50 text-center font-bold text-cyan-600 dark:text-cyan-400 transition-colors">
                        {{ $section->class_name }}
                    </div>
                </div>
            </div>
