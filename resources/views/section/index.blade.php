<x-app-layout>
    <div class="py-8 px-4 sm:px-6 lg:px-8 w-full mx-auto">
        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-8">
            <div>
                <h1 class="text-3xl font-bold text-slate-900 dark:text-white tracking-tight">Gestión de Secciones</h1>
                <p class="text-slate-500 dark:text-slate-400 mt-1">Administra las divisiones de cada clase y sus
                    mentores.</p>
            </div>
            <a href="{{ route('section.create') }}"
                class="inline-flex items-center gap-2 px-5 py-2.5 bg-cyan-600 hover:bg-cyan-500 text-white font-semibold rounded-xl transition-all group text-sm">
        ...
        <div
            ...
            <form action="{{ route('section.index') }}" method="GET" class="flex flex-col md:flex-row items-end gap-4">
                <div class="flex-1 space-y-2">
                    ...
                    <div class="relative group">
                        <select name="classesID" id="classesID"
                            class="w-full pl-6 pr-10 py-3 bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700/50 rounded-2xl text-slate-700 dark:text-slate-200 focus:border-cyan-500/50 focus:ring-4 focus:ring-cyan-500/10 transition-all outline-none appearance-none text-sm"
                ...
                <div class="flex items-center gap-2">
                    ...
                    @endif
                    <button type="submit"
                        class="px-6 py-3 bg-white dark:bg-slate-700/50 hover:bg-slate-50 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 font-bold rounded-2xl transition-all border border-slate-200 dark:border-slate-600/30 text-sm">
                        Buscar
                    </button>
                </div>
            </form>
                ...
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-700/30">
                        @forelse($sections as $section)
                            <tr class="group hover:bg-slate-50 dark:hover:bg-cyan-500/5 transition-all duration-300">
                                <td class="px-6 py-4">
                                    <div
                                        class="font-bold text-slate-800 dark:text-slate-200 group-hover:text-cyan-600 dark:group-hover:text-cyan-400 transition-colors">
                                        {{ $section->section }}
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    <span
                                        class="text-slate-500 dark:text-slate-400 text-sm italic">{{ $section->category }}</span>
                                </td>
                                <td class="px-6 py-4 text-sm">
                                    <div class="flex items-center gap-2">
                                        <div
                                            class="w-12 h-1.5 bg-slate-100 dark:bg-slate-700/50 rounded-full overflow-hidden">
                                            <div class="h-full bg-cyan-500 rounded-full" style="width: 100%"></div>
                                        </div>
                                        <span
                                            class="text-slate-600 dark:text-slate-300 font-mono">{{ $section->capacity }}</span>
                                    </div>
                                </td>
                                <td class="px-6 py-4 text-sm">
                                    <span
