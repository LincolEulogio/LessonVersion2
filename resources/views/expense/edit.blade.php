<x-app-layout>
    <div class="py-12 px-4 sm:px-6 lg:px-8 max-w-3xl mx-auto">
        ...
        <div class="mb-10 flex items-center justify-between">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-slate-500 dark:text-slate-400 text-sm font-medium mt-1">Actualiza los detalles del
                        egreso</p>
                </div>
            </div>
        ...
        <div
            ...
            <form action="{{ route('expense.update', $expense->expenseID) }}" method="POST" class="space-y-8 relative">
                @csrf
                @method('PUT')
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="relative">
                            ...
                            <input type="number" step="0.01" name="amount" id="amount"
                                ...
                                class="w-full pl-8 pr-4 py-3.5 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 focus:border-rose-500 focus:ring-4 focus:ring-rose-500/10 transition-all text-slate-700 dark:text-slate-200 outline-none font-bold">
                        </div>
                        @error('amount')
                            <p class="mt-1 text-xs font-bold text-rose-500">{{ $message }}</p>
                        @enderror
                ...
                <div class="space-y-2">
                    ...
                    <input type="text" name="expense" id="expense" value="{{ old('expense', $expense->expense) }}"
                        required
                        class="w-full px-4 py-3.5 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 focus:border-rose-500 focus:ring-4 focus:ring-rose-500/10 transition-all text-slate-700 dark:text-slate-200 outline-none font-bold">
                    @error('expense')
                        <p class="mt-1 text-xs font-bold text-rose-500">{{ $message }}</p>
                ...
                <div class="space-y-2">
                    ...
                    <textarea name="note" id="note" rows="4"
                        class="w-full px-4 py-3.5 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 focus:border-rose-500 focus:ring-4 focus:ring-rose-500/10 transition-all text-slate-700 dark:text-slate-200 outline-none font-bold resize-none">{{ old('note', $expense->note) }}</textarea>
                    @error('note')
                        <p class="mt-1 text-xs font-bold text-rose-500">{{ $message }}</p>
                    @enderror
                </div>
