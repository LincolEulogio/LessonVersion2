            ...
            <div class="flex items-center gap-6">
                ...
                <div>
                    ...
                    <div class="mt-2 flex items-center gap-3">
                        <span
                            class="px-3 py-1 bg-emerald-600/20 text-emerald-400 border border-emerald-500/20 rounded-lg text-xs font-bold uppercase tracking-wider">
                            {{ $teacher->designation }}
                        </span>
                        <span class="text-slate-500">•</span>
                        <span class="text-slate-400 text-sm italic">Miembro desde:
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div class="space-y-6">
                ...
                <div class="p-6 rounded-3xl bg-slate-800/30 border border-slate-700/50 backdrop-blur-sm">
                    <h3 class="text-sm font-bold text-slate-500 uppercase tracking-widest mb-4">Identificación</h3>
                    <div class="space-y-4">
                        <div class="flex flex-col">
                            <span class="text-xs text-slate-500 uppercase tracking-wider">DNI / Cédula</span>
                            <span class="text-slate-200 font-bold text-lg tracking-widest">{{ $teacher->dni }}</span>
                        </div>
                        <div class="flex flex-col">
                            <span class="text-xs text-slate-500 uppercase tracking-wider">Usuario en Sistema</span>
                            <span class="text-emerald-400 font-mono text-sm">{{ $teacher->username }}</span>
                        </div>
                    </div>
                </div>
                ...
                <div class="p-6 rounded-3xl bg-slate-800/30 border border-slate-700/50 backdrop-blur-sm space-y-4">
                    <h3 class="text-sm font-bold text-slate-500 uppercase tracking-widest mb-2">Canales de Contacto</h3>
                    <div class="flex items-center gap-4 group">
                        ...
                        <div class="flex flex-col overflow-hidden">
                            ...
                            <span class="text-sm text-slate-200 truncate">{{ $teacher->email }}</span>
                        </div>
                    </div>
                    <div class="flex items-center gap-4 group">
                        <div
                        ...
                        <div class="flex flex-col">
                            ...
                            <span class="text-sm text-slate-200">{{ $teacher->phone ?? 'Sin registrar' }}</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="md:col-span-2 space-y-8">
                ...
                <div class="p-8 rounded-3xl bg-slate-800/30 border border-slate-700/50 backdrop-blur-sm">
                    <h2
                        ...
                        <i class="ti ti-user-check text-emerald-400"></i>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-8">
                        <div>
                            ...
                            <span
                                class="text-slate-200 mt-1 block font-semibold text-lg">{{ $teacher->designation }}</span>
                        </div>
                        <div>
                            <span class="block text-xs font-bold text-slate-500 uppercase tracking-widest">Fecha de
                                Nacimiento</span>
                            <span
                                class="text-slate-200 mt-1 block font-medium">{{ \Carbon\Carbon::parse($teacher->dob)->format('d \d\e F, Y') }}</span>
                        </div>
                        <div>
                            <span class="block text-xs font-bold text-slate-500 uppercase tracking-widest">Género</span>
                            <span class="text-slate-200 mt-1 block font-medium">{{ $teacher->sex }}</span>
                        </div>
                        <div>
                            <span
                                class="block text-xs font-bold text-slate-500 uppercase tracking-widest">Antigüedad</span>
                            <span
                                class="text-slate-200 mt-1 block font-medium">{{ \Carbon\Carbon::parse($teacher->jod)->diffForHumans(['parts' => 2]) }}</span>
                        </div>
                        <div class="md:col-span-2">
                            <span
                                class="block text-xs font-bold text-slate-500 uppercase tracking-widest">Residencia</span>
                            <span
                                class="text-slate-200 mt-1 block font-medium leading-relaxed">{{ $teacher->address ?? 'Dirección no registrada en el sistema.' }}</span>
                        </div>
                    </div>
                </div>

                <!-- Academic Load / Subjects - Prototype -->
                <div class="p-8 rounded-3xl bg-slate-800/30 border border-slate-700/50 backdrop-blur-sm opacity-60">
                    <h2
                        class="text-xl font-bold text-white mb-6 border-b border-slate-700/50 pb-4 flex items-center gap-3">
                        <i class="ti ti-book text-emerald-400"></i>
