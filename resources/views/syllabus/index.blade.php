    ...
    <div class="py-10 px-4 sm:px-6 lg:px-8 w-full mx-auto">
        ...
        <div class="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div class="space-y-2">
                <nav class="flex items-center gap-3 text-slate-400 mb-2">
                    ...
                    <i class="ti ti-chevron-right text-xs"></i>
                    <span
                        class="text-xs font-black uppercase tracking-[0.2em] text-emerald-500">{{ __('Plan De Estudios') }}</span>
                </nav>
                ...
                <p class="text-slate-500 dark:text-slate-400 text-sm font-medium uppercase tracking-tighter">
                    {{ __('Gestión y descarga de materiales curriculares por clase') }}
                </p>
            </div>

            @if ($user && $user->hasPermission('plan_de_estudios_add'))
                <div class="flex flex-wrap items-center gap-3">
                    <a href="{{ route('syllabus.create') }}"
                        ...
                        <i class="ti ti-file-plus text-xl transition-transform group-hover:rotate-12"></i>
                        {{ __('Nuevo Plan') }}
                    </a>
                </div>
            @endif
        ...
        <div class="grid grid-cols-1 lg:grid-cols-4 gap-6 mb-10">
            ...
            <div
                ...
                <form action="{{ route('syllabus.index') }}" method="GET"
            ...
            <div
                ...
                <i
                    class="ti ti-file-description absolute -right-4 -bottom-4 text-8xl opacity-10 group-hover:scale-110 transition-transform"></i>
                <p class="text-[10px] font-black uppercase tracking-[0.2em] opacity-80">{{ __('Documentos Totales') }}
                </p>
                <h3 class="text-4xl font-black mt-2">{{ $syllabuses->total() }}</h3>
                <div class="mt-4 flex items-center gap-2">
                    <span
                        class="px-2 py-0.5 bg-white/20 rounded-md text-[10px] font-black uppercase">{{ __('Actualizado') }}</span>
                </div>
            </div>
        </div>
                    ...
                    <thead>
                        <tr
                            ...
                            <th class="px-6 py-6 text-left">
                                <span
                                    class="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">{{ __('Título del Documento') }}</span>
                            </th>
                            <th
                    ...
                    <tbody class="divide-y divide-slate-100 dark:divide-slate-700/50 text-sm">
                        @forelse($syllabuses as $syllabus)
                            <tr
                                ...
                                <td class="px-10 py-6">
                                    <span
                                        class="px-3 py-1 bg-slate-100 dark:bg-slate-800 rounded-lg text-xs font-black text-slate-500 dark:text-slate-400 border border-slate-200 dark:border-slate-700 shadow-sm shrink-0">
                                        #{{ str_pad($syllabus->syllabusID, 3, '0', STR_PAD_LEFT) }}
                                    </span>
                                </td>
                                <td class="px-6 py-6">
                                    <div class="flex items-center gap-4">
                                        <div
                                            ...
                                            @php
                                                ...
                                                $icon = match ($ext) {
                                                    'pdf' => 'ti-file-type-pdf text-rose-500',
                                                    'doc', 'docx' => 'ti-file-type-doc text-blue-500',
                                                    'ppt', 'pptx' => 'ti-file-type-ppt text-amber-500',
                                                    'zip' => 'ti-file-zip text-fuchsia-500',
                                                    'jpg', 'png', 'jpeg' => 'ti-photo text-emerald-500',
                                        ...
                                        <div class="flex flex-col min-w-0">
                                            <span
                                                class="font-black text-slate-700 dark:text-slate-200 truncate pr-4">{{ $syllabus->title }}</span>
                                            <span
                                                class="text-[10px] font-bold text-slate-400 uppercase tracking-tighter">{{ $ext ? strtoupper($ext) : 'N/A' }}</span>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-6">
                                    <span
                                        class="px-3 py-1.5 bg-indigo-50 dark:bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 rounded-xl text-[10px] font-black uppercase tracking-widest border border-indigo-100 dark:border-indigo-500/20 shadow-sm">
                                        {{ $syllabus->class_name }}
                                    </span>
                                </td>
                                <td class="px-6 py-6 max-w-xs">
                                    <p
                                        class="text-xs text-slate-500 dark:text-slate-400 line-clamp-2 italic leading-relaxed">
                                        {{ $syllabus->description ?: __('Sin descripción') }}
                                    </p>
                                </td>
                                <td class="px-6 py-6">
                                    <div class="flex flex-col font-mono">
                                        <span class="text-xs font-black text-slate-700 dark:text-slate-300">
                                            {{ \Carbon\Carbon::parse($syllabus->create_date)->format('d/m/Y') }}
                                        </span>
                                        <span class="text-[10px] text-slate-400 uppercase">
                                            {{ \Carbon\Carbon::parse($syllabus->create_date)->format('H:i A') }}
                                        </span>
                                    </div>
                                </td>
                                <td class="px-10 py-6 text-right">
                                    <div class="flex items-center justify-end gap-2">
                                        @if ($syllabus->file && $user && $user->hasPermission('plan_de_estudios_view'))
                                        ...
                                        @endif
                                        @if ($user && $user->hasPermission('plan_de_estudios_view'))
                                            <a href="{{ route('syllabus.show', $syllabus->syllabusID) }}"
                                        ...
                                        @endif
                                        @if ($user && $user->hasPermission('plan_de_estudios_edit'))
                                        ...
                                        @endif
                                        @if ($user && $user->hasPermission('plan_de_estudios_delete'))
                                            <button
                                                onclick="confirmDelete('{{ $syllabus->syllabusID }}', '{{ $syllabus->title }}')"
                                            ...
                                            <form id="delete-form-{{ $syllabus->syllabusID }}"
                                                action="{{ route('syllabus.destroy', $syllabus->syllabusID) }}"
                                ...
                                <td colspan="6" class="px-10 py-20 text-center">
                                    <div class="flex flex-col items-center justify-center space-y-4">
                                        <div
                                            ...
                                            <i class="ti ti-file-search text-7xl"></i>
                                        </div>
                                        <div class="max-w-xs mx-auto">
                                            ...
                                            <p class="text-slate-500 dark:text-slate-400 text-sm mt-1">
                                                {{ __('No se encontraron planes de estudio para los filtros seleccionados.') }}
                                            </p>
                                        </div>
                                    </div>
                                </td>
                ...
                <div
                    ...
                    <div class="flex flex-col md:flex-row items-center gap-3">
                        ...
                        <div class="flex items-center gap-2">
                            <span
                                class="px-3 py-1 bg-white dark:bg-slate-800 rounded-lg text-xs font-black text-emerald-600 border border-slate-200 dark:border-slate-700 shadow-sm">{{ $syllabuses->firstItem() ?? 0 }}</span>
                            <span class="text-slate-300 dark:text-slate-700">—</span>
                            <span
                                class="px-3 py-1 bg-white dark:bg-slate-800 rounded-lg text-xs font-black text-emerald-600 border border-slate-200 dark:border-slate-700 shadow-sm">{{ $syllabuses->lastItem() ?? 0 }}</span>
                        </div>
                        <span
                            class="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">{{ __('de') }}</span>
                        <span
                            class="text-sm font-black text-slate-900 dark:text-white">{{ $syllabuses->total() }}</span>
                        <span
    ...
    @push('scripts')
        <script>
            function confirmDelete(id, title) {
                Swal.fire({
                    ...
                    text: `Estás a punto de borrar el plan "${title}". Se eliminará el registro y el archivo adjunto de forma permanente.`,
                    icon: 'warning',
                    iconColor: '#f43f5e',
        ...
        <style>
            .premium-pagination nav span[aria-current="page"] span {
                @apply bg-emerald-600 text-white border-emerald-600 shadow-lg shadow-emerald-600/20 rounded-xl px-4 py-2 text-xs font-black !important;
            }

            .premium-pagination nav a {
                @apply bg-white dark:bg-slate-800 text-slate-500 hover:text-emerald-600 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2 text-xs font-black transition-all hover:scale-110 !important;
            }
        </style>
    @endpush
</x-app-layout>
