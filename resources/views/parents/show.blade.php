        ...
        <div class="mb-8 flex items-center justify-between">
            <div class="flex items-center gap-6">
                <img src="{{ asset($parent->photo && $parent->photo != 'default.png' ? 'storage/images/' . $parent->photo : 'uploads/images/default.png') }}"
                    ...
                    alt="{{ $parent->name }}">
                <div>
                    <h1 class="text-3xl font-bold text-slate-900 dark:text-white">{{ $parent->name }}</h1>
                    <div class="mt-2 flex items-center gap-3">
                        <span
                            class="px-3 py-1 bg-sky-600/10 dark:bg-sky-600/20 text-sky-600 dark:text-sky-400 border border-sky-500/20 rounded-lg text-xs font-bold uppercase tracking-wider">
                            Padre de Familia / Tutor
                        </span>
                        <span class="text-slate-300 dark:text-slate-500">•</span>
                        <span class="text-slate-500 dark:text-slate-400 text-sm italic">DNI: {{ $parent->dni }}</span>
                    </div>
                </div>
            </div>
                ...
                <div
                    ...
                    <div class="space-y-4">
                        <div class="flex flex-col">
                            ...
                            <span
                                class="text-slate-700 dark:text-slate-200 font-bold">{{ $parent->father_name ?? 'N/A' }}</span>
                            @if ($parent->father_profession)
                                <span class="text-[10px] text-sky-400 italic">{{ $parent->father_profession }}</span>
                            @endif
                        </div>
                        <div class="flex flex-col pt-2 border-t border-slate-100 dark:border-slate-700/30">
                            <span class="text-xs text-slate-500">Madre</span>
                            <span
                                class="text-slate-700 dark:text-slate-200 font-bold">{{ $parent->mother_name ?? 'N/A' }}</span>
                ...
                <div
                    ...
                    <div class="flex items-center gap-4 group">
                        ...
                        <div class="flex flex-col overflow-hidden">
                            ...
                            <span
                                class="text-sm text-slate-700 dark:text-slate-200 truncate">{{ $parent->email ?? 'Sin correo' }}</span>
                        </div>
                    </div>
                    <div class="flex items-center gap-4 group">
                        <div
                        ...
                        <div class="flex flex-col">
                            ...
                            <span
                                class="text-sm text-slate-700 dark:text-slate-200">{{ $parent->phone ?? 'N/A' }}</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="md:col-span-2 space-y-8">
                <div
                    ...
                    <h2
                        ...
                        <i class="ti ti-home-heart text-sky-600 dark:text-sky-400"></i>
                        Perfil de Tutor
                    </h2>
                    <div class="grid grid-cols-1 gap-8">
                        <div>
                            <span class="block text-xs font-bold text-slate-500 uppercase tracking-widest">Nombre
                                Completo</span>
                            <span
                                class="text-slate-700 dark:text-slate-200 mt-1 block font-semibold text-xl">{{ $parent->name }}</span>
                        </div>
                        <div>
                            <span class="block text-xs font-bold text-slate-500 uppercase tracking-widest">Dirección
                                Registrada</span>
                            <span
                                class="text-slate-700 dark:text-slate-200 mt-1 block font-medium leading-relaxed">{{ $parent->address ?? 'No hay dirección registrada.' }}</span>
                        </div>
                    </div>
                </div>
                ...
                <div
                    ...
                    <h2
                        class="text-xl font-bold text-slate-900 dark:text-white mb-6 border-b border-slate-100 dark:border-slate-700/50 pb-4 flex items-center gap-3">
                        <i class="ti ti-school text-sky-600 dark:text-sky-400"></i>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            @foreach ($parent->students as $student)
                                <a href="{{ route('student.show', $student->studentID) }}"
                                    class="group p-4 bg-white dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700/50 rounded-2xl hover:border-sky-500/50 dark:hover:bg-sky-500/5 shadow-sm dark:shadow-none transition-all flex items-center gap-4">
                                    <div
                                        class="w-12 h-12 rounded-xl bg-slate-50 dark:bg-slate-800 overflow-hidden border border-slate-100 dark:border-slate-700 flex-shrink-0">
                                        <img src="{{ asset($student->photo ? 'storage/images/' . $student->photo : 'uploads/images/default.png') }}"
                                            class="w-full h-full object-cover group-hover:scale-110 transition-transform">
                                    </div>
                                    <div class="overflow-hidden">
                                        <h4
                                            class="text-slate-700 dark:text-slate-200 font-bold text-sm truncate group-hover:text-sky-600 dark:group-hover:text-white transition-colors">
                                            {{ $student->name }}</h4>
                                        <div
