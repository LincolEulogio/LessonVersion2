<x-app-layout>
    <div class="py-12 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
        ...
        <div class="mb-10 flex flex-col md:flex-row md:items-center md:justify-between gap-6">
            <div class="flex items-center gap-4">
                <div
                    ...
                    <i class="ti ti-user-shield text-3xl"></i>
                </div>
                <div>
                    <h1 class="text-3xl font-black text-slate-900 dark:text-white tracking-tight">
                        Miembros de Hospedaje
                    </h1>
                    <p class="text-slate-500 dark:text-slate-400 text-sm font-medium mt-1">Gestión de estudiantes
                        residentes y asignaciones</p>
                </div>
            </div>

            <!-- Class Filter -->
            <form action="{{ route('hmember.index') }}" method="GET"
                ...
                <select name="classID" onchange="this.form.submit()"
                    class="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl px-4 py-2 text-slate-700 dark:text-slate-200 focus:border-rose-500 focus:ring-4 focus:ring-rose-500/10 transition-all outline-none cursor-pointer font-bold text-sm">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            @forelse ($students as $student)
                <div
                    class="group bg-white dark:bg-slate-800/30 border border-slate-200 dark:border-slate-700/50 backdrop-blur-xl rounded-3xl p-6 shadow-sm dark:shadow-none hover:border-rose-500/50 transition-all duration-300 relative overflow-hidden">
                    <div
                        class="absolute -top-10 -right-10 w-32 h-32 bg-rose-500/5 dark:bg-rose-500/10 rounded-full blur-3xl group-hover:bg-rose-500/20 transition-all">
                    </div>
                    ...
                    <div class="flex items-start gap-4 mb-6 relative">
                        ...
                        <div class="flex-1 min-w-0">
                            <h3
                                class="font-black text-slate-900 dark:text-white truncate group-hover:text-rose-600 dark:group-hover:text-rose-400 transition-colors text-lg">
                                {{ $student->name }}
                            </h3>
                            <p
                    ...
                    <div class="space-y-3 mb-6 relative">
                        @if ($student->hmember)
                            <div
                                class="flex items-center gap-3 p-3 rounded-2xl bg-rose-50 dark:bg-rose-500/5 border border-rose-100 dark:border-rose-500/20 transition-colors group-hover:bg-white dark:group-hover:bg-slate-900">
                                <div
                                    class="w-10 h-10 rounded-xl bg-white dark:bg-slate-800 flex items-center justify-center text-rose-500 shadow-sm border border-rose-100 dark:border-rose-500/20">
                                    <i class="ti ti-building-community text-xl"></i>
                                </div>
                                <div>
                                    <p
                                        class="text-[10px] font-black text-rose-400 dark:text-rose-500 uppercase tracking-widest leading-none mb-1">
                                        Hostal Asignado</p>
                                    <p class="font-bold text-slate-700 dark:text-slate-200 text-sm tracking-tight">
                                        {{ $student->hmember->hostel->name }}</p>
                                </div>
                            </div>
                            <div
                                class="flex items-center gap-3 p-3 rounded-2xl bg-amber-50 dark:bg-amber-500/5 border border-amber-100 dark:border-amber-500/20 transition-colors group-hover:bg-white dark:group-hover:bg-slate-900">
                                <div
                                    class="w-10 h-10 rounded-xl bg-white dark:bg-slate-800 flex items-center justify-center text-amber-500 shadow-sm border border-amber-100 dark:border-amber-500/20">
                                    <i class="ti ti-leaf text-xl"></i>
                                </div>
                                <div>
                                    <p
                                        class="text-[10px] font-black text-amber-400 dark:text-amber-500 uppercase tracking-widest leading-none mb-1">
                                        Categoría / Tarifa</p>
                                    <p class="font-bold text-slate-700 dark:text-slate-200 text-sm tracking-tight">
                                        {{ $student->hmember->category->class_type }}
                                        (${{ $student->hmember->hbalance }})</p>
                                </div>
                            </div>
                        @else
                            <div
                    ...
                    <div
                        ...
                        @if ($student->hmember)
                            <div class="flex flex-col">
                                <span
                                    class="text-[9px] font-black text-rose-400 dark:text-rose-500 uppercase tracking-widest">Registrado
                                    desde</span>
                                <span
                                    class="text-[11px] font-bold text-slate-500 dark:text-slate-400">{{ \Carbon\Carbon::parse($student->hmember->hjoindate)->format('d M, Y') }}</span>
                            </div>
                            <form action="{{ route('hmember.destroy', $student->hmember->hmemberID) }}" method="POST"
                                onsubmit="return confirm('¿Retirar al estudiante del hostal?')">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
                        ...
                        @else
                            <button onclick="openModal({{ $student->studentID }}, '{{ $student->name }}')"
                                class="w-full h-11 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white text-[10px] font-black uppercase tracking-widest shadow-lg shadow-indigo-600/20 transition-all flex items-center justify-center gap-2">
                                <i class="ti ti-plus text-base"></i> Registrar Hospedaje
                            </button>
                        @endif
                    </div>
                </div>
        ...
        <div
            ...
            <div
                ...
                <h3
                    ...
                    <i class="ti ti-building-community text-rose-500"></i> Asignar Hospedaje
                </h3>
            ...
            <form action="{{ route('hmember.store') }}" method="POST" class="p-8 space-y-6">
                @csrf
                <input type="hidden" name="studentID" id="modal_studentID">

                <div
                    class="p-4 rounded-2xl bg-rose-50 dark:bg-rose-500/10 border border-rose-100 dark:border-rose-500/20">
                    <p class="text-[10px] font-black text-rose-400 uppercase tracking-widest mb-1">Estudiante</p>
                    <p class="text-lg font-black text-rose-700 dark:text-rose-300" id="modal_studentName"></p>
                </div>
                ...
                <div class="space-y-4">
                    <div class="space-y-2">
                        <label
                            ...
                            Hostal</label>
                        <select name="hostelID" id="modal_hostelID" required onchange="loadCategories(this.value)"
                            class="w-full px-4 py-3 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 focus:border-rose-500 focus:ring-4 focus:ring-rose-500/10 transition-all text-slate-700 dark:text-slate-200 outline-none font-bold">
                            <option value="">Seleccione un hostal...</option>
                            @foreach (\App\Models\Hostel::all() as $h)
                                <option value="{{ $h->hostelID }}">{{ $h->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="space-y-2">
                        <label
                            ...
                            de Habitación (Categoría)</label>
                        <select name="categoryID" id="modal_categoryID" required
                            class="w-full px-4 py-3 rounded-xl bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 focus:border-rose-500 focus:ring-4 focus:ring-rose-500/10 transition-all text-slate-700 dark:text-slate-200 outline-none font-bold">
                            <option value="">Primero seleccione un hostal...</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="pt-4">
                    <button type="submit"
                        class="w-full py-4 rounded-2xl bg-rose-600 hover:bg-rose-500 text-white font-black text-xs uppercase tracking-widest transition-all shadow-lg shadow-rose-600/30 hover:shadow-rose-600/50 hover:scale-[1.02] active:scale-95">
    ...
    <script>
        ...
        async function loadCategories(hostelID) {
            const select = document.getElementById('modal_categoryID');
            select.innerHTML = '<option value="">Cargando...</option>';

            if (!hostelID) {
                select.innerHTML = '<option value="">Primero seleccione un hostal...</option>';
                return;
            }

            try {
                const response = await fetch(`/api/hostel/${hostelID}/categories`);
                ...
                categories.forEach(c => {
                    ...
                    opt.text = `${c.class_type} ($${c.hbalance})`;
                    select.appendChild(opt);
                });
            } catch (error) {
                select.innerHTML = '<option value="">Error al cargar categorías</p>';
            }
        }
    </script>
</x-app-layout>
